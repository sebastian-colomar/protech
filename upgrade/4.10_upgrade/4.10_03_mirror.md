...
3.4. Extract the tar archives containing the directory of the mirrored images and catalogs and its contents:
   ...
   tar fvx ${MIRROR_OCP_REPOSITORY}-${OCP_RELEASE_NEW}.tar

   for RH_INDEX in ${RH_INDEX_LIST}; do
      export MIRROR_OLM_REPOSITORY=mirror-${RH_INDEX}   
      for version in ${RH_INDEX_VERSION_NEW}; do
         cd ${REMOVABLE_MEDIA_PATH}
...
3.7. Upload the release images to the local container registry:
   ...
   oc-${OCP_RELEASE_NEW} image mirror "file://openshift/release:${OCP_RELEASE_NEW}-${ARCH_RELEASE}*" ${LOCAL_REGISTRY}/${MIRROR_OCP_REPOSITORY}-${OCP_RELEASE_NEW} --from-dir=${REMOVABLE_MEDIA_PATH}/${MIRROR_OCP_REPOSITORY}-${OCP_RELEASE_NEW} --insecure
...
3.8. Create the mirrored release image signature ConfigMap manifest:
   ...
   ```
   oc-${OCP_RELEASE_OLD} apply -f ${REMOVABLE_MEDIA_PATH}/${MIRROR_OCP_REPOSITORY}-${OCP_RELEASE_NEW}/config/signature-sha256-*.yaml
...
3.9. Create the ImageContentSourcePolicy manifest:
   ...
   ```
   oc-${OCP_RELEASE_OLD} apply -f ${REMOVABLE_MEDIA_PATH}/${MIRROR_OCP_REPOSITORY}-${OCP_RELEASE_NEW}/config/icsp.yaml
