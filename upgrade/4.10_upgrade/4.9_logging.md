...
1.7. Verify that the Kibana pod is in the ready status:
...
1.8. Update the current custom catalog source of the `elasticsearch-operator` operator to use the custom mirror catalog as shown:
    ...
    oc patch subscription elasticsearch-operator -n openshift-storage --type json --patch '[{"op": "replace", "path": "/spec/source", "value": "'${CATALOG_SOURCE}'" }]'
...
1.15. Verify that the Kibana pod is in the ready status:
...
1.16. Update the current custom catalog source of the `cluster-logging` operator to use the custom mirror catalog as shown:
    ...
    oc patch subscription cluster-logging -n local-storage --type json --patch '[{"op": "replace", "path": "/spec/source", "value": "'${CATALOG_SOURCE}'" }]'
...
1.23. Verify that the Kibana pod is in the ready status:
...
1.31. Verify that the Kibana pod is in the ready status:
...
1.39. Verify that the Kibana pod is in the ready status:
