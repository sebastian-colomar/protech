...
3.6. Upload the release images to the local container registry:
   ...
   ```
   for release in ${OCP_RELEASE_NEW} ${OCP_RELEASE_OLD}; do
      oc-${release} image mirror "file://openshift/release:${release}-${ARCH_RELEASE}*" ${LOCAL_REGISTRY}/${OCP_REPOSITORY} --from-dir=${REMOVABLE_MEDIA_PATH}/${OCP_REPOSITORY}-${release} --insecure
   done
   ```
