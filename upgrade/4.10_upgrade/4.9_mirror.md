...
3.10. Upload the catalog images to the local container registry:
   ...
   for RH_INDEX in ${RH_INDEX_LIST}; do
      ...
      for release in ${RH_INDEX_VERSION_NEW} ${RH_INDEX_VERSION_OLD}; do
         ...
         oc-${release} adm catalog mirror file://${MIRROR_OLM_REPOSITORY}-${release}/${RH_REPOSITORY}/${RH_INDEX}:${release} ${LOCAL_REGISTRY}/${MIRROR_OLM_REPOSITORY}-${release} --insecure
      done
   done
...
3.11. Create the CatalogSource object by running the following command to specify the catalogSource.yaml file in your manifests directory:
   ...
   for RH_INDEX in ${RH_INDEX_LIST}; do
      ...
      for release in ${RH_INDEX_VERSION_NEW} ${RH_INDEX_VERSION_OLD}; do
         ...
         oc-${OCP_RELEASE_OLD} apply -f ${REMOVABLE_MEDIA_PATH}/${MIRROR_OLM_REPOSITORY}-${release}/manifests-${RH_INDEX}-*/catalogSource.yaml
      done
   done
...
3.12. Create the ImageContentSourcePolicy (ICSP) object by running the following command to specify the imageContentSourcePolicy.yaml file in your manifests directory:
   ...
   for RH_INDEX in ${RH_INDEX_LIST}; do
      ...
      for release in ${RH_INDEX_VERSION_NEW} ${RH_INDEX_VERSION_OLD}; do
         cd ${REMOVABLE_MEDIA_PATH}/${MIRROR_OLM_REPOSITORY}-${release}
         oc-${release} adm catalog mirror ${LOCAL_REGISTRY}/${MIRROR_OLM_REPOSITORY}-${release}/${RH_REPOSITORY}-${RH_INDEX}:${release} ${LOCAL_REGISTRY}/${MIRROR_OLM_REPOSITORY}-${release} --insecure --manifests-only
         ...
         oc-${OCP_RELEASE_OLD} apply -f ${REMOVABLE_MEDIA_PATH}/${MIRROR_OLM_REPOSITORY}-${release}/manifests-${RH_REPOSITORY}-${RH_INDEX}-*/imageContentSourcePolicy.yaml
      done
   done
