...
3.6. Upload the release images to the local container registry:
   ...
   ```
   for release in ${OCP_RELEASE_NEW} ${OCP_RELEASE_OLD}; do
      oc-${release} image mirror "file://openshift/release:${release}-${ARCH_RELEASE}*" ${LOCAL_REGISTRY}/${OCP_REPOSITORY}-${release} --from-dir=${REMOVABLE_MEDIA_PATH}/${OCP_REPOSITORY}-${release} --insecure
   done
   ```
...
3.11. Upload the catalog images to the local container registry:
   ...
   ```
   ```
   oc-${OCP_RELEASE_NEW} adm catalog mirror ${LOCAL_REGISTRY}/${OLM_REPOSITORY}-${OCP_RELEASE_NEW}/${RH_REPOSITORY}-${RH_INDEX}:${RH_INDEX_VERSION_NEW} ${LOCAL_REGISTRY}/${OLM_REPOSITORY}-${OCP_RELEASE_NEW} --insecure --manifests-only
   ```
   ```
   ...
   ```
   ```
   oc-${OCP_RELEASE_OLD} adm catalog mirror ${LOCAL_REGISTRY}/${OLM_REPOSITORY}-${OCP_RELEASE_OLD}/${RH_REPOSITORY}-${RH_INDEX}:${RH_INDEX_VERSION_OLD} ${LOCAL_REGISTRY}/${OLM_REPOSITORY}-${OCP_RELEASE_OLD} --insecure --manifests-only
   ```
