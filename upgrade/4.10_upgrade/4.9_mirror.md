...
3.11. Upload the catalog images to the local container registry and apply the corresponding CatalogSource resource:
   ```
...
3.12. Create the ImageContentSourcePolicy (ICSP) object by running the following command to specify the imageContentSourcePolicy.yaml file in your manifests directory:
   ```
   for release in ${RH_INDEX_VERSION_NEW} ${RH_INDEX_VERSION_OLD}; do
      oc-${OCP_RELEASE_OLD} apply -f ${REMOVABLE_MEDIA_PATH}/${MIRROR_OLM_REPOSITORY}-${release}/manifests-${RH_INDEX}-*/imageContentSourcePolicy.yaml
   done
   ```

3.13. Allow HTTP connections to the mirror registry:
