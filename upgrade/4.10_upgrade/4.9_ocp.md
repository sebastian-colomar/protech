...
1.5. Mirror the images and configuration manifests to a directory on the removable media:

   ```
   for release in ${OCP_RELEASE_NEW} ${OCP_RELEASE_OLD}; do
      sudo mkdir -p ${REMOVABLE_MEDIA_PATH}/${MIRROR_OCP_REPOSITORY}-${release}
      sudo chown -R ${USER}. ${REMOVABLE_MEDIA_PATH}
      alias oc=oc-${release} && oc version
      oc adm release mirror -a ${LOCAL_SECRET_JSON} quay.io/${PRODUCT_REPO}/${RELEASE_NAME}:${release}-${ARCH_RELEASE} --to-dir=${REMOVABLE_MEDIA_PATH}/${MIRROR_OCP_REPOSITORY}-${release}
   done

   ```

1.6. Retrieve the ImageContentSourcePolicy:

   ```
   for release in ${OCP_RELEASE_NEW} ${OCP_RELEASE_OLD}; do
      alias oc=oc-${release} && oc version
      oc adm release mirror -a ${LOCAL_SECRET_JSON} quay.io/${PRODUCT_REPO}/${RELEASE_NAME}:${release}-${ARCH_RELEASE} --to=${LOCAL_REGISTRY}/${MIRROR_OCP_REPOSITORY}-${release} --to-release-image=${LOCAL_REGISTRY}/${MIRROR_OCP_REPOSITORY}-${release}:${release}-${ARCH_RELEASE} --insecure --dry-run | tee ${REMOVABLE_MEDIA_PATH}/${MIRROR_OCP_REPOSITORY}-${release}/config/icsp.yaml
...
1.7. Create a tar archive containing the directory and its contents:
...
1.8. Upload the release and the openshift client tarball to the mirror host:
   ```
