...
2.5. Inspect the `${RH_INDEX}-${version}.txt` file and identify which package names from this list you want to keep in your pruned index.
   ...
   ```
   alias oc=oc-${OCP_RELEASE_OLD} && oc version
...
2.7. Run the following command to prune the source index of all but the specified packages and push the new index image to your target registry:
   ...
   ln -s ${HOME}/bin/opm-${OCP_RELEASE_NEW} ${HOME}/bin/opm-${RH_INDEX_VERSION_NEW}
   ln -s ${HOME}/bin/opm-${OCP_RELEASE_OLD} ${HOME}/bin/opm-${RH_INDEX_VERSION_OLD}
   ...
   for RH_INDEX in ${RH_INDEX_LIST}; do
      ...
      for version in ${RH_INDEX_VERSION_NEW} ${RH_INDEX_VERSION_OLD}; do
         ...
         alias opm=opm-${version} && opm version
...
2.9. Run the following command on your workstation with unrestricted network access to mirror the content to local files:
   ```
   ln -s ${HOME}/bin/oc-${OCP_RELEASE_NEW} ${HOME}/bin/oc-${RH_INDEX_VERSION_NEW}
   ln -s ${HOME}/bin/oc-${OCP_RELEASE_OLD} ${HOME}/bin/oc-${RH_INDEX_VERSION_OLD}
   ...
   for RH_INDEX in ${RH_INDEX_LIST}; do
      ...
      for version in ${RH_INDEX_VERSION_NEW} ${RH_INDEX_VERSION_OLD}; do
         ...
         alias oc=oc-${version} && oc version
         oc adm catalog mirror ${INDEX_IMAGE_PRUNED} file://${MIRROR_OLM_REPOSITORY}-${version} -a ${LOCAL_SECRET_JSON} --index-filter-by-os=linux/${ARCH_CATALOG} --insecure
