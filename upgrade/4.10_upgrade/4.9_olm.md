...
2.1. Set up environment variables:
   ...
   export RHINDEX_VERSION_OLD=v4.8
...
2.9. Run the following command on your workstation with unrestricted network access to mirror the content to local files:
   ...
   oc-${OCP_RELEASE_NEW} adm catalog mirror ${INDEX_IMAGE_PRUNED_NEW} file://${MIRROR_OLM_REPOSITORY}-${RH_INDEX_VERSION_NEW} -a ${LOCAL_SECRET_JSON} --index-filter-by-os=linux/${ARCH_CATALOG} --insecure
   ```
   ```
   cd ${REMOVABLE_MEDIA_PATH}/${MIRROR_OLM_REPOSITORY}-${RH_INDEX_VERSION_OLD}
   oc-${OCP_RELEASE_OLD} adm catalog mirror ${INDEX_IMAGE_PRUNED_OLD} file://${MIRROR_OLM_REPOSITORY}-${RH_INDEX_VERSION_OLD} -a ${LOCAL_SECRET_JSON} --index-filter-by-os=linux/${ARCH_CATALOG} --insecure
   ```
