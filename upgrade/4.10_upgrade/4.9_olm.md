...
2.6. Run the following command to prune the source index of all but the specified packages:
   ...
   alias opm=opm-${OCP_RELEASE_NEW}
   opm index prune -f ${INDEX_IMAGE_NEW} -p "${PKGS}" -t ${INDEX_IMAGE_PRUNED_NEW}

   alias opm=opm-${OCP_RELEASE_OLD}
   opm index prune -f ${INDEX_IMAGE_OLD} -p "${PKGS}" -t ${INDEX_IMAGE_PRUNED_OLD}
