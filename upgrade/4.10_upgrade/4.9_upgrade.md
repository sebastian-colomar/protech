...
4.6. Update the cluster:
   ...
   ```
   alias oc=oc-${OCP_RELEASE_OLD}
   oc adm upgrade --allow-explicit-upgrade --to-image ${LOCAL_REGISTRY}/${MIRROR_OCP_REPOSITORY}-${OCP_RELEASE_NEW}@sha256:${SHA256_SUM_VALUE}

   ```

4.7. Force an explicit upgrade with version set:
