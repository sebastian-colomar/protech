...
1.6. Mirror the images and configuration manifests to a directory on the removable media:
   ...
   ```
   for release in ${OCP_RELEASE_NEW} ${OCP_RELEASE_OLD}; do
      mkdir -p ${REMOVABLE_MEDIA_PATH}/${OCP_REPOSITORY}-${release}
      chown -R ${USER}. ${REMOVABLE_MEDIA_PATH}
...
1.9. Upload the generated tarball to the mirror host:
   ...
   ```
   ```
   ssh -i ${SSH_KEY} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${REMOTE_USER}@${MIRROR_HOST} "mkdir -p ${REMOVABLE_MEDIA_PATH}"
   ssh -i ${SSH_KEY} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${REMOTE_USER}@${MIRROR_HOST} "chown ${REMOTE_USER}. ${REMOVABLE_MEDIA_PATH}"
   for release in ${OCP_RELEASE_NEW} ${OCP_RELEASE_OLD}; do
...
2.6. Run the following command to prune the source index of all but the specified packages:
   ...
   export INDEX_IMAGE_PRUNED_NEW=localhost:5000/${RH_REPOSITORY}/${RH_INDEX}:${RH_INDEX_VERSION_NEW}
   export INDEX_IMAGE_PRUNED_OLD=localhost:5000/${RH_REPOSITORY}/${RH_INDEX}:${RH_INDEX_VERSION_OLD}
   export MIRROR_PORT=5000
   export MIRROR_PROTOCOL=http
   export MIRROR_HOST=mirror.hub.sebastian-colomar.com
   ...
   ```
   ```
   semanage fcontext -a -t container_file_t "/mnt/mirror/containers/containers(/.*)?"
   semanage fcontext -a -t container_var_run_t "/mnt/mirror/containers/containers-run(/.*)?"
   restorecon -Rv /mnt/mirror/containers/containers /mnt/mirror/containers/containers-run
   ```
   ```
   mkdir -p ${HOME}/.docker
...
2.8. Run the following command to push the new index image to your target registry:
   ...
   ```
   ```
   podman tag ${INDEX_IMAGE_NEW} ${INDEX_IMAGE_PRUNED_NEW}
   podman tag ${INDEX_IMAGE_OLD} ${INDEX_IMAGE_PRUNED_OLD}
   ```
   ```
...
2.9. Run the following command on your workstation with unrestricted network access to mirror the content to local files:
   ...
   ```
   ```
   oc-${OCP_RELEASE_NEW} adm catalog mirror ${INDEX_IMAGE_PRUNED_NEW} file://${OLM_REPOSITORY}-{OCP_RELEASE_NEW} -a ${LOCAL_SECRET_JSON} --index-filter-by-os=linux/${ARCH_CATALOG} --insecure --dir ${REMOVABLE_MEDIA_PATH}/${OLM_REPOSITORY}-{OCP_RELEASE_NEW}
   oc-${OCP_RELEASE_OLD} adm catalog mirror ${INDEX_IMAGE_PRUNED_OLD} file://${OLM_REPOSITORY}-{OCP_RELEASE_OLD} -a ${LOCAL_SECRET_JSON} --index-filter-by-os=linux/${ARCH_CATALOG} --insecure --dir ${REMOVABLE_MEDIA_PATH}/${OLM_REPOSITORY}-{OCP_RELEASE_OLD}
   ```
...
3.6. Upload the release images to the local container registry:
   ...
   ```
   mkdir -p ${REMOVABLE_MEDIA_PATH}/${OCP_REPOSITORY}
   chown -R ${USER}. ${REMOVABLE_MEDIA_PATH}
   ```
   ```
...
4.10. Some troubleshooting hints:
   ...
   ```
   ```
   curl -s ${MIRROR_PROTOCOL}://${LOCAL_REGISTRY}/v2/ocp/tags/list | jq .
   ```
   ```
   curl -s ${MIRROR_PROTOCOL}://${LOCAL_REGISTRY}/v2/ocp/tags/list | jq -r '.tags[]'
   ```
   ```
   for tag in $( curl -s ${MIRROR_PROTOCOL}://${LOCAL_REGISTRY}/v2/ocp/tags/list | jq -r '.tags[]' );do
      curl -sIH 'Accept: application/vnd.docker.distribution.manifest.v2+json' ${MIRROR_PROTOCOL}://${LOCAL_REGISTRY}/v2/${OCP_REPOSITORY}/manifests/${tag} | awk /Docker-Content-Digest/'{print "'${tag}'",$2}'
   done
   ```
   ```
   ls ${REMOVABLE_MEDIA_PATH}/${CONTAINER_NAME}/docker/registry/v2/repositories/${OCP_REPOSITORY}/_manifests/revisions/sha256/
   ```
   ```
   podman exec ${CONTAINER_NAME} ls ${CONTAINER_VOLUME}/docker/registry/v2/repositories/${OCP_REPOSITORY}/_manifests/revisions/sha256/
   ```
---
