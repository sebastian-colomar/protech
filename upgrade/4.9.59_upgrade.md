...
6. Run the following command to prune the source index of all but the specified packages:
   ...
   export PRUNED_INDEX_IMAGE=localhost:5000/${RH_REPOSITORY}/${RH_INDEX}:${RH_INDEX_VERSION}
   ```
   ```
   mkdir -p ${HOME}/.docker
...
1. Set the required environment variables:
   ...
   c. Export the local repository name for the release images mirror:
   ...
   c. Export the local repository name for the catalog images mirror:
