...
3. Set the required environment variables:
   ...
   ```
5. Install the OpenShift CLI by downloading the binary:
   ...
   sudo ln -s oc-${OCP_RELEASE_NEW} oc
   ```
...
4. Mirror the images and configuration manifests to a directory on the removable media:
   ...
   oc adm release mirror -a ${LOCAL_SECRET_JSON} quay.io/${PRODUCT_REPO}/${RELEASE_NAME}:${OCP_RELEASE_NEW}-${ARCHITECTURE} --to-dir=${REMOVABLE_MEDIA_PATH}/${OCP_REPOSITORY}
   ```

3. Retrieve the ImageContentSourcePolicy:

   ```
   oc adm release mirror -a ${LOCAL_SECRET_JSON} quay.io/${PRODUCT_REPO}/${RELEASE_NAME}:${OCP_RELEASE_NEW}-${ARCHITECTURE} --to=${LOCAL_REGISTRY}/${OCP_REPOSITORY} --to-release-image=${LOCAL_REGISTRY}/${OCP_REPOSITORY}:${OCP_RELEASE_NEW}-${ARCHITECTURE} --insecure --dry-run | tee ${REMOVABLE_MEDIA_PATH}/${OCP_REPOSITORY}/config/icsp.yaml
   ...
   ```
    for release in ${OCP_RELEASE_NEW} ${OCP_RELEASE_OLD}; do
      for package in ${PACKAGES}; do
         ...
         ssh -i ${SSH_KEY} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${REMOTE_USER}@${MIRROR_HOST} "sudo mv -fv ${package}-linux-${release}.tar.gz /root"
      done
    done
   ```
...
1. Disable the sources for the default catalogs by adding `disableAllDefaultSources: true` to the OperatorHub object:
   ```
...
2. Run the source index image that you want to prune in a container:
   ```
...
3. Use the grpcurl command to get a list of the packages provided by the index:
   ```
...
4. Inspect the `index.txt` file and identify which package names from this list you want to keep in your pruned index.
5. Alternatively, list your current subscriptions:
   ```
   export PKGS=$(oc get subscriptions -A -o jsonpath='{range .items[*]}{.spec.name}{"\n"}{end}' | sort -u | paste -sd, -)
   ```
6. Run the following command to prune the source index of all but the specified packages:
   ```
   ...
   ```
7. Deploy the local container registry using the Distribution container image with the HTTP protocol:
   ```
   ...
   ```
8. Run the following command to push the new index image to your target registry:
   ```
   ...
   sudo podman push ${INDEX_IMAGE_PRUNED}   
   ```
9. Run the following command on your workstation with unrestricted network access to mirror the content to local files:
   ```
   mkdir -p ${REMOVABLE_MEDIA_PATH}/${OLM_REPOSITORY}
   oc-${OCP_RELEASE_OLD} adm catalog mirror ${INDEX_IMAGE_PRUNED} file://${OLM_REPOSITORY} -a ${LOCAL_SECRET_JSON} --index-filter-by-os=linux/${ARCHITECTURE} --insecure --dir ${REMOVABLE_MEDIA_PATH}/${OLM_REPOSITORY}
   ```
9. Copy the directory that is generated in your current directory to removable media:
   ```
   cd ${REMOVABLE_MEDIA_PATH}
   sudo tar cfv ${OLM_REPOSITORY}.tar ${OLM_REPOSITORY}
   ```
8. Upload the generated tarball to the mirror host:
   ```
   export MIRROR_HOST=mirror.sebastian-colomar.com
   export REMOTE_USER=ec2-user
   export SSH_KEY=${HOME}/key.txt
...
5. Extract the tar archive containing the directory of the mirrored catalogs and its contents:
...
4. Upload the release images to the local container registry:

   ```
   sudo mkdir -p ${REMOVABLE_MEDIA_PATH}/${OCP_REPOSITORY}
   sudo chown -R ${USER}. ${REMOVABLE_MEDIA_PATH}

   oc image mirror "file://openshift/release:${OCP_RELEASE_NEW}*" ${LOCAL_REGISTRY}/${OCP_REPOSITORY} --from-dir=${REMOVABLE_MEDIA_PATH}/${OCP_REPOSITORY} --insecure
   ```

1. Print the mirrored release image signature ConfigMap manifest to standard output:
...
1. Print the ImageContentSourcePolicy manifest to standard output:
...
8. Upload the catalog images to the local container registry:
   ```
   ...
   ```
8. Create the ImageContentSourcePolicy (ICSP) object by running the following command to specify the imageContentSourcePolicy.yaml file in your manifests directory:
   ```
...
1. Validate that the ImageContentSourcePolicy has been rendered into a MachineConfig and successfully rolled out to all nodes before proceeding:
   ```
   export MIRROR_HOST=mirror.hub.sebastian-colomar.com
...
3. Upgrading to an OCP version higher than 4.8 requires manual acknowledgment from the administrator. For more information, see Preparing to upgrade to OpenShift Container Platform 4.9:
...
5. Force an explicit upgrade with version set (ONLY IF NECESSARY):
...
8. Watch the CVO logs while it downloads/unpacks:
...
# 4. Upgrade OCS to Red Hat OpenShift Data Foundation (ODF) 4.9
