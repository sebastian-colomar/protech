...
1.4. Mirror the images and configuration manifests to a directory on the removable media:
...
1.3. Retrieve the ImageContentSourcePolicy:
...
1.5. Create a tar archive containing the directory and its contents:
...
1.5. Upload the generated tarball to the mirror host:
   ```
...
1.5. Upload the openshift client tarball to the mirror host:
   ```
    for release in ${OCP_RELEASE_NEW} ${OCP_RELEASE_OLD}; do
      for package in ${PACKAGES}; do
...
2.9. Copy the directory that is generated in your current directory to removable media:
   ```
   cd ${REMOVABLE_MEDIA_PATH}
   sudo tar cfv ${OLM_REPOSITORY}.tar ${OLM_REPOSITORY}
   ```

2.8. Upload the generated tarball to the mirror host:
   ```
   export MIRROR_HOST=mirror.sebastian-colomar.com
   export REMOTE_USER=ec2-user
   export SSH_KEY=${HOME}/key.txt
...
3.1. Deploy the local container registry using the Distribution container image with the HTTP protocol:
   ```
   export CONTAINER_IMAGE=docker.io/library/registry
   export CONTAINER_IMAGE_TAG=2.7
   export CONTAINER_NAME=registry
...
3.5. Extract the tar archive containing the directory of the mirrored images and its contents:
...
3.4. Upload the release images to the local container registry:
...
3.1. Print the mirrored release image signature ConfigMap manifest to standard output:
...
3.1. Print the ImageContentSourcePolicy manifest to standard output:
...
3.8. Upload the catalog images to the local container registry:
   ```
...
3.8. Create the ImageContentSourcePolicy (ICSP) object by running the following command to specify the imageContentSourcePolicy.yaml file in your manifests directory:
   ```
...
4.1. Retrieve the sha256 sum value for the release from the image signature ConfigMap:
...
4.2. To Select the stable-4.9 channel, run this patch command on the CLI:

   ```
   oc patch clusterversion version --type merge -p '{"spec": {"channel": "stable-4.9"}}'
   ```

4.3. Upgrading to an OCP version higher than 4.8 requires manual acknowledgment from the administrator. For more information, see Preparing to upgrade to OpenShift Container Platform 4.9:
...
4.5. Force an explicit upgrade with version set (ONLY IF NECESSARY):
