...
2.6. Run the following command to prune the source index of all but the specified packages:
   ...
   sudo -E podman info | egrep 'graphRoot:|runRoot:|imageCopyTmpDir:'
   ```
   ```
   sudo semanage fcontext -a -t container_file_t "/mnt/mirror/containers/containers(/.*)?"
   sudo semanage fcontext -a -t container_var_run_t "/mnt/mirror/containers/containers-run(/.*)?"
   sudo restorecon -Rv /mnt/mirror/containers/containers /mnt/mirror/containers/containers-run
   ```
   ```
   mkdir -p ${HOME}/.docker
   ...
   ```
   ```
   opm index prune -f ${INDEX_IMAGE} -p "${PKGS}" -t ${INDEX_IMAGE_PRUNED}
   ```
...
2.8. Run the following command to push the new index image to your target registry:
   ...
   ```
   ```
   podman tag ${INDEX_IMAGE} ${INDEX_IMAGE_PRUNED}
   ```
   ```
...
4.10. Some troubleshooting hints:
   ...
   ```
   ```
   sudo podman exec ${CONTAINER_NAME} ls ${CONTAINER_VOLUME}/docker/registry/v2/repositories/${OCP_REPOSITORY}/_manifests/revisions/sha256/
   ```
---
