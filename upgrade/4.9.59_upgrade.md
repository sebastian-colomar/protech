...
4. Mirror the images and configuration manifests to a directory on the removable media:
   ...
   ```
   ```
   oc adm release mirror -a ${LOCAL_SECRET_JSON} quay.io/${PRODUCT_REPO}/${RELEASE_NAME}:${OCP_RELEASE_NEW}-${ARCHITECTURE} --to-dir=${REMOVABLE_MEDIA_PATH}/${LOCAL_REPOSITORY}
   ```

3. Retrieve the ImageContentSourcePolicy:

   ```
   oc adm release mirror -a ${LOCAL_SECRET_JSON} quay.io/${PRODUCT_REPO}/${RELEASE_NAME}:${OCP_RELEASE_NEW}-${ARCHITECTURE} --to=${LOCAL_REGISTRY}/${LOCAL_REPOSITORY} --to-release-image=${LOCAL_REGISTRY}/${LOCAL_REPOSITORY}:${OCP_RELEASE_NEW}-${ARCHITECTURE} --insecure --dry-run | tee ${REMOVABLE_MEDIA_PATH}/${LOCAL_REPOSITORY}/config/icsp.yaml
...
4. Upload the images to the local container registry:
   ...
   ```
   ```
   oc image mirror "file://openshift/release:${OCP_RELEASE_NEW}*" ${LOCAL_REGISTRY}/${LOCAL_REPOSITORY} --from-dir=${REMOVABLE_MEDIA_PATH}/${LOCAL_REPOSITORY} --insecure
   ```
...
1. Print the ImageContentSourcePolicy manifest to standard output:
   ...
   ```
   cat ${REMOVABLE_MEDIA_PATH}/${LOCAL_REPOSITORY}/config/icsp.yaml
   ```
...
5. Force an explicit upgrade with version set (ONLY IF NECESSARY):
   ...
   ```
   #oc patch clusterversion version --type=merge -p '{"spec":{"desiredUpdate":{"image":"'${LOCAL_REGISTRY}/${LOCAL_REPOSITORY}@sha256:${SHA256_SUM_VALUE}'","version":"'${OCP_RELEASE_NEW}'","force":true}}}'
   ```
