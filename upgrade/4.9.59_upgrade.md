...
2.9. Run the following command on your workstation with unrestricted network access to mirror the content to local files:
   ...
   ```
   ```
   oc-${OCP_RELEASE_NEW} adm catalog mirror ${INDEX_IMAGE_PRUNED_NEW} file://${REMOVABLE_MEDIA_PATH}/${OLM_REPOSITORY}-${OCP_RELEASE_NEW} -a ${LOCAL_SECRET_JSON} --index-filter-by-os=linux/${ARCH_CATALOG} --insecure
   oc-${OCP_RELEASE_OLD} adm catalog mirror ${INDEX_IMAGE_PRUNED_OLD} file://${REMOVABLE_MEDIA_PATH}/${OLM_REPOSITORY}-${OCP_RELEASE_OLD} -a ${LOCAL_SECRET_JSON} --index-filter-by-os=linux/${ARCH_CATALOG} --insecure
   ```
