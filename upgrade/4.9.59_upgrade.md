...
5. Force an explicit upgrade with version set (ONLY IF NECESSARY):
   ...
   ```
   #oc patch clusterversion version --type=merge -p '{"spec":{"desiredUpdate":{"image":"'${LOCAL_REGISTRY}/${LOCAL_REPOSITORY}@sha256:${SHA256_SUM_VALUE}'","version":"'${OCP_RELEASE}'","force":true}}}'
   ```
